...
| **country** | VARCHAR(50) | The country of residence for the customer (e.g., 'Germany', 'United States'). |
...
| **create_date** | DATE | The date and time when the customer record was created in the source system. |

---

### 2. **gold.dim_products**

* **Purpose:** Provides information about the products, their costs, and categories.
* **Columns:**
...
| **product_number** | VARCHAR(50) | A structured alphanumeric code representing the product (e.g., 'BK-R93R-62'). |
...
| **maintenance** | VARCHAR(50) | Indicates specific maintenance requirements for the product. |
...
| **product_key** | INT | Surrogate key linking the order to the product dimension table (`dim_products`). |
| **customer_key** | INT | Surrogate key linking the order to the customer dimension table (`dim_customers`). |
